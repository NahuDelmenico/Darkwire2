                    ...
                    <div class="bg-dark rounded p-4 border border-warning">
                            ...
                            <div class="cart-item border-bottom border-secondary pb-4 mb-4" data-item-id="{{ $item['id'] ?? $index }}">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-3 col-sm-4 mb-3 mb-md-0">
                                        <img src="{{ $item['imagen'] ?? 'https://via.placeholder.com/200x100?text=Game+Image' }}" 
                                             class="img-fluid rounded" 
                                             alt="{{ $item['nombre'] ?? 'Juego' }}"
                                             style="max-height: 120px; object-fit: cover;">
                                    </div>
                                    
                                    <!-- Información del Juego -->
                                    <div class="col-md-5 col-sm-8">
                                        <h5 class="text-white mb-2">{{ $item['nombre'] ?? 'Nombre del Juego' }}</h5>
                                        ...
                                        <p class="text-muted mb-0">
                                            <small>{{ $item['descripcion'] ?? 'Descripción del juego...' }}</small>
                                        </p>
                                    </div>
                                    
                                    <!-- Cantidad y Precio -->
                                    <div class="col-md-2 col-sm-6 text-center mb-3 mb-md-0">
                                        <label class="text-muted small">Cantidad</label>
                                        <div class="input-group input-group-sm">
                                            <button class="btn btn-outline-warning btn-decrease" type="button" data-item-id="{{ $item['id'] ?? $index }}">
                                                <i class="bi bi-dash"></i>
                                            </button>
                                            <input type="number" class="form-control text-center quantity-input" 
                                                   value="{{ $item['cantidad'] ?? 1 }}" min="1" 
                                                   data-item-id="{{ $item['id'] ?? $index }}">
                                            <button class="btn btn-outline-warning btn-increase" type="button" data-item-id="{{ $item['id'] ?? $index }}">
                        ...
                        @else
                            ...
                            <div class="text-center py-5" id="empty-cart">
                                ...
                                <a href="{{ route('inicio') }}" class="btn btn-warning btn-lg">

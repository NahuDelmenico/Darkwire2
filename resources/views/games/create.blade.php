        ...
        <div class="d-flex justify-content-center">
            <form action="{{route('games.store')}}" class="w-75" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    @error('discount') arial-invalid="true" arial-errornessage="error-discount" @enderror value="{{old('discount')}}">
                    @error('discount')
            ...
            <div class="mb-3">
                ...
                <select 
                    ...
                    @enderror
                >
                    <option value="" disabled {{ old('gamemode_fk') ? '' : 'selected' }}>Seleccione una categoría</option>
                    @foreach($gamemodes as $g)
                        <option value="{{ $g->gamemode_id }}" {{ old('gamemode_fk') == $g->gamemode_id ? 'selected' : '' }}>
                            {{ $g->name }}
                        </option>
                    @endforeach
                </select>
